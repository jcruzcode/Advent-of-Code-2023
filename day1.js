/* The function calibrationSum takes in a string input containing lines of text with calibration values embedded in them, extracts the calibration value from each line, and returns the sum of all calibration values */
function calibrationSum(input) {
  const lines = input.split('\n'); // Split in to array by newlines
  const digits = ['1', '2', '3', '4', '5', '6', '7', '8', '9'];
  let nums = [], num, current, solo;

  for (let line of lines) {
    num = [];

    for (let i = 0; i < line.length; i++) {
      /* if the current character in line is found in the digits array
      add that character to the num array */
      if (digits.includes(line[i])) num.push(line[i]);
    }

    if ( num.length > 1 ) {
      // Current number is first and last digit combined
      current = +( num.shift() + num.pop() );
    } else {
      // If only one digit, current number is that digit twice
      solo = num.pop();
      current = +`${solo}${solo}`;
    }
    // Add the current number to the nums array
    nums.push(current);
  }
...
/* The function extractDigits takes a string argument and parses the string for digits in the form of words and single digits then returns the first and last digits combined */
function extractDigits(string) {
  ...
  // Mutiple digits represented as one word
  const words = [
    ['eightwo', '8 2'],
    ['oneight', '1 8'],
    ['threeight', '3 8'],
  ...
  const nums = [];
  let copy = string.slice(); // Copy of string
  let chars;

  /* Loop over words array and replace all occurences of word in copy with a    string of space separated digits */
  for (let i = 0; i < words.length; i++) {
    if (copy.includes(words[i][0])) {
      copy = copy.replaceAll(words[i][0], ` ${words[i][1]}`)
    }
  }

  /* Loop over digits array and replace all occurences of digit with the same   digit but with a single space before and two spaces after */
  for (let digit of digits) {
    if (copy.includes(digit)) {
      copy = copy.replaceAll(digit, ` ${digit}  `);
    }
  }

  /* Trim the space at the beginning and end of copy and split the string by a single   space */
  chars = copy.trim().split(' ');

  /* Loop over chars array and if the char is included in the digits array add the    
  char to the nums array */
  ...
  /* Loop over nums array and if the digit is a word replace it with its        number as a string */
  for (let i = 0; i < nums.length; i++) {
    for (let item in map) {
      if (nums[i] === item) nums[i] = map[item];
    }
  }

  /* Return the first and last digit combined to form a single two-digit        number */
  if (nums.length > 1) {
...
function sum(input) {
  // Separate the string input into individual strings based on newlines 
  const strings = input.split('\n');
  let total = 0;

  for (let string of strings) {
    /* 
    extractDigits will take the current string and return a two digit             number. The two digit number returned will be added to the running total      */
    total += extractDigits(string);
  }
